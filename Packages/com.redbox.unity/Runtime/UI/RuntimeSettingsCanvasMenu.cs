...
public class RuntimeSettingsCanvasMenu : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Auto-find d'un controller gameplay par nom de type si pauseTargets est vide.
        if (pauseTargets == null || pauseTargets.Length == 0)
        {
            Behaviour gameplayController = FindBehaviourByTypeName("FPSController");
            if (gameplayController != null)
                pauseTargets = new[] { gameplayController };
        }
    ...
    private static Behaviour FindBehaviourByTypeName(string typeName)
    {
        ...
        for (int i = 0; i < all.Length; i++)
        {
            ...
            if (candidate.GetType().Name.Equals(typeName, StringComparison.Ordinal))
                return candidate;
        }
